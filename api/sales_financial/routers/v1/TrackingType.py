...
from api.sales_financial.services.TrackingTypeService import TrackingTypeService
from api.sales_financial.schemas.TrackingTypeSchema import (
    TrackingTypeBase, 
    CreateTrackingType,
    TrackingTypeSchema
)
...
async def list(skip: int=0, limit: int=100, typeService: TrackingTypeService = Depends()):
...
async def get(code: int, typeService: TrackingTypeService = Depends()):
    trackingtype = await typeService.getbycode(code=code)
    if trackingtype is None:
        raise HTTPException(type_code=status.HTTP_404_NOT_FOUND, detail="Tracking Type not found")
    return trackingtype
...
async def create(data: List[CreateTrackingType], typeService: TrackingTypeService = Depends()):
...
async def update(code: int, data: TrackingTypeBase, typeService: TrackingTypeService = Depends()):
