...
from api.sales_financial.services.ContactTypeService import ContactTypeService
from api.sales_financial.schemas.ContactTypeSchema import (
    ContactTypeBase, 
    CreateContactType,
    ContactTypeSchema
)
...
async def list(skip: int=0, limit: int=100, typeService: ContactTypeService = Depends()):
...
async def get(code: int, typeService: ContactTypeService = Depends()):
...
async def create(data: List[CreateContactType], typeService: ContactTypeService = Depends()):
...
async def update(code: int, data: ContactTypeBase, typeService: ContactTypeService = Depends()):
