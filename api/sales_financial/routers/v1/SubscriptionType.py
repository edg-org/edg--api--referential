...
from api.sales_financial.services.SubscriptionTypeService import SubscriptionTypeService
from api.sales_financial.schemas.SubscriptionTypeSchema import (
...
async def list(skip: int=0, limit: int=100, typeService: SubscriptionTypeService = Depends()):
...
async def get(code: int, typeService: SubscriptionTypeService = Depends()):
    subscriptiontype = await typeService.getbycode(code=code)
    if subscriptiontype is None:
        raise HTTPException(type_code=status.HTTP_404_NOT_FOUND, detail="Subscription Type not found")
...
async def create(data: List[CreateSubscriptionType], typeService: SubscriptionTypeService = Depends()):
...
async def update(code: int, data: SubscriptionTypeBase, typeService: SubscriptionTypeService = Depends()):
