...
from api.sales_financial.models.SubscriptionStatusModel import SubscriptionStatusModel
from api.sales_financial.repositories.SubscriptionStatusRepo import SubscriptionStatusRepo
from api.sales_financial.schemas.SubscriptionStatusSchema import SubscriptionStatusBase, CreateSubscriptionStatus

class SubscriptionStatusService:
    subscriptionstatus: SubscriptionStatusRepo

    def __init__(
        self, subscriptionstatus: SubscriptionStatusRepo = Depends()
    ) -> None:
    ...
    async def getbycode(self, code: str) -> SubscriptionStatusBase:
        return self.subscriptionstatus.getbycode(code=code)
    
    # get subscription status by name function
    async def getbyname(self, name: str) -> SubscriptionStatusBase:
    ...
    async def create(self, data: List[CreateSubscriptionStatus]) -> List[CreateSubscriptionStatus]:
        for item in data:
            ...
            if subscriptionstatus:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Subscription Status already registered with code "+ str(item.code))
            
            subscriptionstatus = self.subscriptionstatus.getbyname(name=item.name)
            if subscriptionstatus:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Subscription Status already registered with name "+ item.name)
        
        return self.subscriptionstatus.create(data=data)

    # update subscription status function
    async def update(self, code: int, data: SubscriptionStatusBase) -> SubscriptionStatusModel:
        subscriptionstatus = self.subscriptionstatus.get(code=code)
    ...
    async def delete(self, subscription: SubscriptionStatusModel) -> None:
        subscriptionstatus = self.subscriptionstatus.get(id=id)
        if subscriptionstatus is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Subscription Status not found")
        
        self.subscriptionstatus.update(subscription)
        return HTTPException(status_code=status.HTTP_200_OK, detail="Subscription Status deleted")