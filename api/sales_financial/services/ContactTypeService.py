...
from api.sales_financial.models.ContactTypeModel import ContactTypeModel
from api.sales_financial.repositories.ContactTypeRepo import ContactTypeRepo
from api.sales_financial.schemas.ContactTypeSchema import ContactTypeBase, CreateContactType
...
class ContactTypeService:
    ...
    async def create(self, data: List[CreateContactType]) -> List[CreateContactType]:
        for item in data:
            ...
            if contacttype:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Contact Type already registered with code "+ str(item.code))
            
            contacttype = self.contacttype.getbyname(name=item.name)
            if contacttype:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Contact Type already registered with name "+ item.name)
        
        return self.contacttype.create(data=data)

    # update contact type function
    async def update(self, code: int, data: ContactTypeBase) -> ContactTypeModel:
    ...
    async def delete(self, contact: ContactTypeModel) -> None:
        ...
        return HTTPException(status_code=status.HTTP_200_OK, detail="Contact Type deleted")