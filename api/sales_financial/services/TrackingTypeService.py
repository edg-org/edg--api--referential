...
from api.sales_financial.models.TrackingTypeModel import TrackingTypeModel
from api.sales_financial.repositories.TrackingTypeRepo import TrackingTypeRepo
from api.sales_financial.schemas.TrackingTypeSchema import TrackingTypeBase, CreateTrackingType
...
class TrackingTypeService:
    ...
    async def getbycode(self, code: str) -> TrackingTypeBase:
        return self.trackingtype.getbycode(code=code)
    
    # get tracking type by name function
    async def getbyname(self, name: str) -> TrackingTypeBase:
    ...
    async def create(self, data: List[CreateTrackingType]) -> List[CreateTrackingType]:
        for item in data:
            ...
            if trackingtype:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Tracking Type already registered with code "+ str(item.code))
            
            trackingtype = self.trackingtype.getbyname(name=item.name)
            if trackingtype:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Tracking Type already registered with name "+ item.name)
        
        return self.trackingtype.create(data=data)

    # update tracking type function
    async def update(self, code: int, data: TrackingTypeBase) -> TrackingTypeModel:
        trackingtype = self.trackingtype.get(code=code)
        if trackingtype is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Tracking Type not found")
    ...
    async def delete(self, tracking: TrackingTypeModel) -> None:
        ...
        return HTTPException(status_code=status.HTTP_200_OK, detail="Tracking Type deleted")