...
from api.geographical.schemas.AreaSchema import CreateArea
...
class AreaRepo:
    ...
    def maxcode_bycity(self, city_code: int) -> int:
        return self.db.query(func.max(AreaModel.code)).where(AreaModel.infos["city_code"] == city_code).one()[0]
    
    # get area id by code function
    def getid_bycode(self, code: int) -> AreaModel:
        return self.db.query(AreaModel.id).where(AreaModel.code == code).one()[0]
    
    # get all areas function
    def list(self, skip: int = 0, limit: int = 100) -> List[AreaModel]:
        return self.db.query(AreaModel).offset(skip).limit(limit).all()

    # get area by id function
    def get(self, id: int) -> AreaModel:
        return self.db.query(AreaModel).where(AreaModel.id == id).first()
    
    # get area code function
    def getbycode(self, code: str) -> AreaModel:
        return self.db.query(AreaModel).where(AreaModel.code == code).first()
    
    # get area name function
    def getbyname(self, name: str) -> AreaModel:
        return self.db.query(AreaModel).where(AreaModel.infos["name"] == name).first()
    
    # create area function
    def create(self, data: List[CreateArea]) -> List[CreateArea]:
        self.db.execute(insert(AreaModel), encoders.jsonable_encoder(data))
        self.db.commit()
        return data
