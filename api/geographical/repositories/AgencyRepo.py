...
from api.geographical.schemas.AgencySchema import CreateAgency
...
class AgencyRepo:
    ...
    def maxcode_byzone(self, area_code: int) -> int:
        return self.db.query(func.max(AgencyModel.code)).where(AgencyModel.infos["area_code"] == area_code).one()[0]
    
    # get agency id by code function
    def getid_bycode(self, code: int) -> AgencyModel:
        return self.db.query(AgencyModel.id).where(AgencyModel.code == code).one()[0]
    
    # get all agencys function
    def list(self, skip: int = 0, limit: int = 100) -> List[AgencyModel]:
        return self.db.query(AgencyModel).offset(skip).limit(limit).all()

    # get agency by id function
    def get(self, id: int) -> AgencyModel:
        return self.db.query(AgencyModel).where(AgencyModel.id == id).first()
    
    # get agency code function
    def getbycode(self, code: str) -> AgencyModel:
        return self.db.query(AgencyModel).where(AgencyModel.code == code).first()
    
    # get agency name function
    def getbyname(self, name: str) -> AgencyModel:
        return self.db.query(AgencyModel).where(func.lower(AgencyModel.infos["name"]) == name.lower()).first()
    
    # create agency function
    def create(self, data: List[CreateAgency]) -> List[CreateAgency]:
        self.db.execute(insert(AgencyModel), encoders.jsonable_encoder(data))
        self.db.commit()
        return data
