...
from api.geographical.schemas.NaturalZoneSchema import CreateZone
...
class ZoneRepo:
    ...
    def maxcode(self) -> int:
        return self.db.query(func.max(ZoneModel.code)).one()[0]
        
    # get all regions function
    def list(self, skip: int = 0, limit: int = 100) -> List[ZoneModel]:
        return self.db.query(ZoneModel).offset(skip).limit(limit).all()

    # get region by id function
    def get(self, id: int) -> ZoneModel:
        return self.db.query(ZoneModel).where(ZoneModel.id == id).first()
    
    # get region id by code function
    def getid_bycode(self, code: int) -> ZoneModel:
        return self.db.query(ZoneModel.id).where(ZoneModel.code == code).one()[0]
    
    # get region by code function
    def getbycode(self, code: int) -> ZoneModel:
        return self.db.query(ZoneModel).where(ZoneModel.code == code).first()
    
    # get region by name function
    def getbyname(self, name: str) -> ZoneModel:
        return self.db.query(ZoneModel).where(func.lower(ZoneModel.name) == name.lower()).first()
    
    # create region function
    def create(self, data: List[CreateZone]) -> List[ZoneModel]:
        self.db.execute(insert(ZoneModel), encoders.jsonable_encoder(data))
        self.db.commit()
        return data
