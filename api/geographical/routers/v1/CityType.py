...
from api.geographical.services.CityTypeService import CityTypeService
from api.geographical.schemas.CityTypeSchema import (
    CityTypeBase, 
    CreateCityType,
    CityTypeSchema
)
...
async def list(skip: int=0, limit: int=100, typeService: CityTypeService = Depends()):
...
async def get(code: int, typeService: CityTypeService = Depends()):
...
async def create(data: List[CreateCityType], typeService: CityTypeService = Depends()):
...
async def update(code: int, data: CityTypeBase, typeService: CityTypeService = Depends()):
