...
from api.geographical.services.CityLevelService import CityLevelService
from api.geographical.schemas.CityLevelSchema import (
    CityLevelBase, 
    CreateCityLevel,
    CityLevelSchema
)
...
async def list(skip: int=0, limit: int=100, levelService: CityLevelService = Depends()):
...
async def get(code: int, levelService: CityLevelService = Depends()):
...
async def create(data: List[CreateCityLevel], levelService: CityLevelService = Depends()):
...
async def update(code: int, data: CityLevelBase, levelService: CityLevelService = Depends()):
