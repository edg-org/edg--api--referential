...
from api.geographical.services.PrefectureService import PrefectureService
from api.geographical.schemas.PrefectureSchema import (
    PrefectureSchema, 
    PrefectureBase, 
    CreatePrefecture,
...
async def list(skip: int=0, limit: int=100, prefectureService: PrefectureService = Depends()):
...
async def get(code: int, prefectureService: PrefectureService = Depends()):
...
async def get_prefecture_item(code: int, prefectureService: PrefectureService = Depends()):
    prefecture = await prefectureService.getbycode(code=code)
    if prefecture is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Prefecture not found")
    return prefecture
...
async def create(data: List[CreatePrefecture], prefectureService: PrefectureService = Depends()):
...
async def update(code: int, data: PrefectureBase, prefectureService: PrefectureService = Depends()):
