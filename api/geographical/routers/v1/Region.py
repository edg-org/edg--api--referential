...
from api.geographical.services.RegionService import RegionService
from api.geographical.schemas.RegionSchema import (
    RegionBase,
    RegionSchema, 
    CreateRegion,
    RegionItemSchema
...
async def list(skip: int=0, limit: int=100, regionService: RegionService = Depends()):
...
async def get(code: int, regionService: RegionService = Depends()):
...
async def get_region_item(code: int, regionService: RegionService = Depends()):
    region = await regionService.getbycode(code=code)
    if region is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Administrative Region not found")
    return region
...
async def create(data: List[CreateRegion], regionService: RegionService = Depends()):
...
async def update(code: int, data: RegionBase, regionService: RegionService = Depends()):
