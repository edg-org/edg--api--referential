...
from api.geographical.services.AgencyService import AgencyService
from api.geographical.schemas.AgencySchema import (
    AgencyBase,
    CreateAgency,
    AgencySchema,
)
...
async def list(skip: int=0, limit: int=100, agencyService: AgencyService = Depends()):
...
async def get(code: int, agencyService: AgencyService = Depends()):
...
async def create(data: List[CreateAgency], agencyService: AgencyService = Depends()):
...
async def update(code: int, data: AgencyBase, agencyService: AgencyService = Depends()):
