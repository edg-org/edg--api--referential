...
from api.geographical.services.AreaTypeService import AreaTypeService
from api.geographical.schemas.AreaTypeSchema import (
    AreaTypeBase, 
    CreateAreaType,
    AreaTypeSchema
)
...
async def list(skip: int=0, limit: int=100, typeService: AreaTypeService = Depends()):
...
async def get(code: int, typeService: AreaTypeService = Depends()):
...
async def create(data: List[CreateAreaType], typeService: AreaTypeService = Depends()):
...
async def update(code: int, data: AreaTypeBase, typeService: AreaTypeService = Depends()):
