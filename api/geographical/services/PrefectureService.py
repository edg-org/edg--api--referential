...
from api.geographical.repositories.RegionRepo import RegionRepo
from api.geographical.models.PrefectureModel import PrefectureModel
from api.geographical.repositories.PrefectureRepo import PrefectureRepo
from api.geographical.schemas.PrefectureSchema import PrefectureBase, CreatePrefecture
...
class PrefectureService:
    ...
    async def create(self, data: List[CreatePrefecture]) -> List[CreatePrefecture]:
        step=0
        ...
        for item in data:            
            step +=1
            item.region_id = RegionRepo.getid_bycode(self.prefecture, item.infos.region_code)
            basecode = prefecture_basecode(item.infos.region_code)                
            prefecture_code = basecode+step
            item.code = prefecture_code
            item.prefecture_number = str((prefecture_code%100)).zfill(2)

            prefecture = self.prefecture.getbycode(code=prefecture_code)
            if prefecture:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Prefecture already registered with code "+ str(item.code))
            
            prefecture = self.prefecture.getbyname(name=item.infos.name)
            if prefecture:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Prefecture already registered with name "+ item.infos.name)
        
        return self.prefecture.create(data=data)

    # update prefecture function
    async def update(self, code: int, data: PrefectureBase) -> PrefectureModel:
        prefecture = self.prefecture.getbycode(code=code)
        if prefecture is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Prefecture not found")
    ...
    async def activate_desactivate(self, id: int, flag: bool) -> None:
