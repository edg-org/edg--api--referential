...
from api.geographical.models.AgencyModel import AgencyModel
from api.geographical.repositories.AgencyRepo import AgencyRepo
from api.geographical.schemas.AgencySchema import AgencyBase, CreateAgency
...
class AgencyService:
    ...
    async def list(self, skip: int = 0, limit: int = 100) -> List[AgencyModel]:
    ...
    async def create(self, data: List[CreateAgency]) -> List[CreateAgency]:
        for item in data:
            item.code = agency_basecode(item.infos)
            agency = self.agency.getbycode(code=item.code)
            if agency:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Agency already registered with code "+ str(item.code))
            
            agency = self.agency.getbyname(name=item.infos.name)
            if agency:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Agency already registered with name "+ item.infos.name)
            
        return self.agency.create(data=data)

    # update agency function
    async def update(self, code: int, data: AgencyBase) -> AgencyModel:
    ...
    async def delete(self, agency: AgencyModel) -> None:
        ...
        return HTTPException(status_code=status.HTTP_200_OK, detail="Agency deleted")