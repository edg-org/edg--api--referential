...
from api.geographical.schemas.CityLevelSchema import CityLevelBase, CreateCityLevel
...
class CityLevelService:
    ...
    async def create(self, data: List[CreateCityLevel]) -> List[CreateCityLevel]:
        for item in data:
            ...
            if citylevel:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="City Level already registered with code "+ str(item.code))
            
            citylevel = self.citylevel.getbyname(name=item.name)
            if citylevel:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="City Level already registered with name "+ item.name)
        
        return self.citylevel.create(data=data)

    # update city level function
    async def update(self, code: int, data: CityLevelBase) -> CityLevelModel:
        citylevel = self.citylevel.getbycode(code=code)
        if citylevel is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="City Level not found")
    ...
    async def delete(self, city: CityLevelModel) -> None:
        citylevel = self.citylevel.get(id=id)
        if citylevel is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="City Level not found")
        
        self.citylevel.update(city)
        return HTTPException(status_code=status.HTTP_200_OK, detail="City Level deleted")