...
from api.geographical.models.CityTypeModel import CityTypeModel
from api.geographical.repositories.CityTypeRepo import CityTypeRepo
from api.geographical.schemas.CityTypeSchema import CityTypeBase, CreateCityType
...
class CityTypeService:
    ...
    async def create(self, data: List[CreateCityType]) -> List[CreateCityType]:
        for item in data:
            ...
            if citytype:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="City Type already registered with code "+ str(item.code))
            
            citytype = self.citytype.getbyname(name=item.name)
            if citytype:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="City Type already registered with name "+ item.name)
        
        return self.citytype.create(data=data)

    # update city type function
    async def update(self, code: int, data: CityTypeBase) -> CityTypeModel:
        citytype = self.citytype.getbycode(code=code)
        if citytype is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="City Type not found")
    ...
    async def delete(self, city: CityTypeModel) -> None:
        ...
        return HTTPException(status_code=status.HTTP_200_OK, detail="City Type deleted")