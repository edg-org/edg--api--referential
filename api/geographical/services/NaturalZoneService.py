...
from api.geographical.models.NaturalZoneModel import ZoneModel
from api.geographical.repositories.NaturalZoneRepo import ZoneRepo
from api.geographical.schemas.NaturalZoneSchema import ZoneBase, CreateZone
...
class ZoneService:
    ...
    async def create(self, data: List[CreateZone]) -> List[CreateZone]:
        ...
        for item in data:
            ...
            if zone:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Natural Region already registered with code "+ str(item.code))
            
            zone = self.zone.getbyname(name=item.name)
            if zone:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Natural Region already registered with name "+ str(item.name))

        return self.zone.create(data=data)

    # update natural region function
    async def update(self, id: int, data: ZoneBase) -> ZoneModel:
        zone = self.zone.getbycode(code=id)
        if zone is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Natural Region not found")
