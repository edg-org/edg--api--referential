...
from api.geographical.models.RegionModel import RegionModel
from api.geographical.repositories.RegionRepo import RegionRepo
from api.geographical.repositories.NaturalZoneRepo import ZoneRepo
from api.geographical.schemas.RegionSchema import RegionBase, CreateRegion
...
class RegionService:
    ...
    async def create(self, data: List[CreateRegion]) -> List[CreateRegion]:
        step=0
        zone_code = 0
        for item in data:
            maxcode = self.region.maxcode_byzone(item.infos.zone_code)
            if maxcode is None:
                maxcode = 0
            
            item.zone_id = ZoneRepo.getid_bycode(self.region, item.infos.zone_code)
            
            if maxcode > 0:
                step +=1
                basecode = maxcode
            else:
                basecode = region_basecode(item.infos.zone_code)
            ...
            if region:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Administrative Region already registered with code "+ str(item.code))
            
            region = self.region.getbyname(name=item.infos.name)
            if region:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Administrative Region already registered with name "+ str(item.infos.name))
        
        return self.region.create(data=data)

    # update region function
    async def update(self, code: int, data: RegionBase) -> RegionModel:
        region = self.region.getbycode(code=code)
        if region is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Administrative Region not found")
