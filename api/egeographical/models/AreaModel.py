...
class AreaModel(EntityMeta):
    ...
    type_id = Column(
        ...
        ForeignKey("area_types.id"),
        nullable=False,
    )
    zipcode = Column(
        String(5), index=True, unique=False, nullable=False
    )
    ...
    area_type = relationship(
        "AreaTypeModel", back_populates="areas"
    )
    agencies = relationship(
        "AgencyModel", back_populates="area"
    )

    energy_departures = relationship(
        "EnergyDepartureModel", back_populates="area"
    )
    transformers = relationship(
        "TransformerModel", back_populates="area"
    )
    connection_points = relationship(
        "ConnectionPointModel", back_populates="area"
    )
    delivery_points = relationship(
        "DeliveryPointModel", back_populates="area"
    )
