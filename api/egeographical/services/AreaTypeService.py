...
from api.egeographical.models.AreaTypeModel import (
    AreaTypeModel,
)
from api.egeographical.repositories.AreaTypeRepo import (
    AreaTypeRepo,
)
from api.egeographical.schemas.AreaTypeSchema import (
    AreaTypeBase,
    CreateAreaType,
)
...
class AreaTypeService:
    ...
    async def create(
        ...
        for item in data:
            ...
            areatype = self.areatype.getbyname(
                name=item.name
            )
            if areatype:
                raise HTTPException(
    ...
    async def update(
        self, code: int, data: AreaTypeBase
    ) -> AreaTypeModel:
