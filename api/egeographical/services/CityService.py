...
from api.egeographical.repositories.CityRepo import CityRepo
from api.tools.Helper import city_basecode, generate_zipcode
from api.egeographical.repositories.CityTypeRepo import (
    CityTypeRepo,
)
from api.egeographical.schemas.CitySchema import (
    CityBase,
    CreateCity,
)
from api.egeographical.repositories.CityLevelRepo import (
    CityLevelRepo,
)
...
class CityService:
    ...
    async def create(
        ...
        for item in data:
            ...
            item.type_id = CityTypeRepo.getbyname(
                self.city, item.infos.type
            ).id
            item.level_id = CityLevelRepo.getbyname(
                self.city, item.infos.level
            ).id
            item.prefecture_id = PrefectureRepo.getidbycode(
                self.city, item.infos.prefecture_code
            )
            ...
            if maxzipcode is not None:
                ...
                if (
                    str(item.infos.level).lower()
                    == "préfecture"
                ):
                    zipcode_step = 0
                else:
                    if (
                        hasattr(item, "zipcode") is False
                    ) or (
                        hasattr(item, "zipcode")
                        and item.zipcode is None
                    ):
                        if zipcode_step == 0:
                            zipcode_step = 110
                        else:
            ...
            if city:
                raise HTTPException(
                    ...
                    detail="City already registered with code "
                    + str(item.code),
                )
            ...
            if city:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="City already registered with name "
                    + item.infos.name,
                )

            city = self.city.getbyzipcode(
                zipcode=item.zipcode
            )
            if city:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="City already registered with zip code "
                    + str(item.zipcode),
                )
