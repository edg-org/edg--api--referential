...
from api.sales.models.InvoiceStatusModel import InvoiceStatusModel
from api.sales.repositories.InvoiceStatusRepo import InvoiceStatusRepo
from api.sales.schemas.InvoiceStatusSchema import InvoiceStatusBase, CreateInvoiceStatus
...
class InvoiceStatusService:
    ...
    async def list(self, skip: int = 0, limit: int = 100) -> List[InvoiceStatusModel]:
        return self.invoicestatus.list(skip=skip, limit=limit)
    ...
    async def getbycode(self, code: str) -> InvoiceStatusBase:
        return self.invoicestatus.getbycode(code=code)
    
    # get invoice status by name function
    async def getbyname(self, name: str) -> InvoiceStatusBase:
        return self.invoicestatus.getbyname(name=name)

    # create invoice status function
    async def create(self, data: List[CreateInvoiceStatus]) -> List[CreateInvoiceStatus]:
        for item in data:
            invoicestatus = self.invoicestatus.getbycode(code=item.code)
            if invoicestatus:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invoice Status already registered with code "+ str(item.code))
            
            invoicestatus = self.invoicestatus.getbyname(name=item.name)
            if invoicestatus:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invoice Status already registered with name "+ item.name)
        
        return self.invoicestatus.create(data=data)

    # update invoice status function
    async def update(self, code: int, data: InvoiceStatusBase) -> InvoiceStatusModel:
    ...
    async def delete(self, invoice: InvoiceStatusModel) -> None:
        ...
        return HTTPException(status_code=status.HTTP_200_OK, detail="Invoice Status deleted")