...
class MeterDeliveryPointHistoryModel(EntityMeta):
    ...
    meter_number = Column(MEDIUMINT(unsigned=True), index=True, unique=True, nullable=False)
    delivery_point_number = Column(MEDIUMINT(unsigned=True), index=True, unique=True, nullable=False)
    installation_date = Column(Date, nullable=False)
    uninstallation_date = Column(Date, nullable=False)
    created_at = Column(DateTime(timezone=True), server_default=datetime.utcnow().isoformat(), nullable=False)
    UniqueConstraint("meter_number", "delivery_point_number", "installation_date", name="uix1_mdph")