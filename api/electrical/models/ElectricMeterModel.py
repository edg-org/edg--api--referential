...
class ElectricMeterModel(EntityMeta):
    ...
    metric_number = Column(
        String(15), index=True, unique=True, nullable=False
    )
    power_mode_id = Column(
        TINYINT(unsigned=True),
        ForeignKey("power_modes.id"),
        nullable=False,
    )
    type_id = Column(
        TINYINT(unsigned=True),
    ...
    meter_type = relationship(
        "MeterTypeModel", back_populates="electric_meters"
    )
    power_mode = relationship(
        "PowerModeModel", back_populates="electric_meters"
    )
